...
/* 其他常量 */
#define SCAN_LIST_SIZE 10 // WiFi扫描结果最大数量
    ...
    if (lv_keyboard_get_selected_btn(target) == 39)
    {
        ...
        WiFiManager::getInstance().connect(ssid, password, [app, ssid, password](bool success)
            ...
            if (success) {
                ...
                WiFiManager::getInstance().saveCredentials(ssid, password);
            } else {
...
{
    ESP_LOGI(TAG, "WiFi switch turned ON1111");
    ...
    { // 开关开启
        ...
        ESP_LOGI(TAG, "WiFi switch turned ON");
        // 若在扫描页则立即启动扫描
        if (app->_screen_index == UI_WIFI_SCAN_INDEX)
        {
            app->startWifiScan();
    ...
    { // 开关关闭
        ...
        ESP_LOGI(TAG, "WiFi switch turned OFF");
        // 若在扫描页则停止扫描
        if (app->_screen_index == UI_WIFI_SCAN_INDEX)
        {
            app->stopWifiScan();
